...
logger = get_logger("jobs.train_return")
...
def run(symbol: str | None = None, timeframe: str | None = None) -> str:
    ...
    job_name = f"train-return:{target}:{tf}"
    ...
    try:
        ...
        out_dir = build_version_dir(settings, "return", target, metadata, timeframe=tf)
        model.save(out_dir)
        ...
        logger.exception("train_return_failed", symbol=target, timeframe=tf, error=str(exc))
        raise
