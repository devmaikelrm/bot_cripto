...
logger = get_logger("jobs.train_risk")
...
def run(symbol: str | None = None, timeframe: str | None = None) -> str:
    ...
    job_name = f"train-risk:{target}:{tf}"
    notifier.notify_job_start(job_name)

    try:
        df = load_feature_dataset(settings, target, timeframe=tf)

        model = BaselineModel(objective="risk")
        metadata = model.train(df, target_col="close")
        out_dir = build_version_dir(settings, "risk", target, metadata, timeframe=tf)
        model.save(out_dir)
        ...
        logger.exception("train_risk_failed", symbol=target, timeframe=tf, error=str(exc))
        raise
