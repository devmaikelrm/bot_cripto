...
def test_risk_engine_blocks_high_vol_regime() -> None:
    ...
    decision = engine.evaluate(prediction=pred, regime=MarketRegime.HIGH_VOL, state=state)
...
def test_risk_engine_position_size_positive() -> None:
    ...
    decision = engine.evaluate(prediction=pred, regime=MarketRegime.TREND, state=state)
